...
class CardGame
  ...
  dif highest_card(card1 card2) # typo: dif should read def; the parameters must be seperated with a comma: (card1, card2). Method not properly indented; highest card should be a class method: self.highest_card
...
def self.cards_total(cards) #This whole method should be indented
  ...
  for card in cards
    total += card.value
    return "You have a total of" + total # This line of code needs to be underneath the end statement on line 32. As it is, it'll keep looping over itself and won't return a total card value. Total is an int and must be converted to string
  end
end
```
